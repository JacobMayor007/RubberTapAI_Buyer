...
export default function ChangePassword({
  ...
  const handleSend = async () => {
    try {
      ...
      await account.createRecovery({
        email,
        url: "https://rubbertapai.netlify.app/forgot-password",
      });
  ...
  return (
    <SafeAreaView className="flex-1">
      <ImageBackground
        className="flex-1 p-7 bg-black/90"
        source={require("@/assets/images/FPBGImage.png")}
      >
        <MaterialIcons
        ...
        <View className="flex-col items-center justify-center">
          ...
          <AppText color={"light"} className="my-1 text-center px-6 font-light">
            Enter the email address you registered with. An email containing a
            link to reset your password will be sent to you.
          </AppText>
          <TextInput
            placeholder="Email"
            placeholderTextColor={"#F3E0C1"}
            className="bg-[#10B981]/50 w-full my-2 rounded-md px-2 text-slate-100"
            value={email}
          ...
          <Pressable
            ...
            className={`${sent ? `bg-[#eaeaea]` : `bg-[#6B8E23]`} mt-10 py-3 items-center w-full rounded-lg`}
          >
